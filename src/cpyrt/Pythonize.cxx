...
//-----------------------------------------------------------------------------
bool HasAttrDirect(PyObject* pyclass, PyObject* pyname,
                   bool mustBecpyrt = false) {
  // prevents calls to Py_TYPE(pyclass)->tp_getattr, which is unnecessary for
  // our purposes here and could tickle problems w/ spurious lookups into ROOT
  // meta
  ...
  if (dct) {
    ...
    Py_DECREF(dct);
    if (attr) {
      bool ret = !mustBecpyrt || CPPOverload_Check(attr);
      Py_DECREF(attr);
      return ret;
    }
  }
...
//-----------------------------------------------------------------------------
inline bool IsTemplatedSTLClass(const std::string& name,
                                const std::string& klass) {
  // Scan the name of the class and determine whether it is a template
  // instantiation.
  auto pos = name.find(klass);
  return pos == 5 && name.rfind("std::", 0, 5) == 0 &&
         name.find("::", name.rfind(">")) == std::string::npos;
}
...
inline PyObject* CallPyObjMethod(PyObject* obj, const char* meth) {
  ...
  Py_INCREF(obj);
  PyObject* result =
      PyObject_CallMethod(obj, const_cast<char*>(meth), const_cast<char*>(""));
  Py_DECREF(obj);
  return result;
}
...
inline PyObject* CallPyObjMethod(PyObject* obj, const char* meth,
  ...
  Py_INCREF(obj);
  PyObject* result = PyObject_CallMethod(obj, const_cast<char*>(meth),
                                         const_cast<char*>("O"), arg1);
  Py_DECREF(obj);
  return result;
}
...
//-----------------------------------------------------------------------------
inline bool AdjustSlice(const Py_ssize_t nlen, Py_ssize_t& start,
...
//-----------------------------------------------------------------------------
inline PyObject* CallSelfIndex(CPPInstance* self, PyObject* idx,
                               PyObject* pymeth) {
  ...
  PyObject* result =
      PyObject_CallMethodOneArg((PyObject*)self, pymeth, pyindex);
  Py_DECREF(pyindex);
  Py_DECREF((PyObject*)self);
  return result;
}
...
PyObject* DeRefGetAttr(PyObject* self, PyObject* name) {
  ...
  if (name == PyStrings::gTypeCode || name == PyStrings::gCTypesType) {
    // TODO: these calls come from TemplateProxy and are unlikely to be needed
...
struct IterItemGetter : public ItemGetter {
  ...
  PyObject* get() override {
    return (*(Py_TYPE(fPyObject)->tp_iternext))(fPyObject);
  }
};
...
static bool FillVector(PyObject* vecin, PyObject* args, ItemGetter* getter) {
  ...
  // two main options: a list of lists (or tuples), or a list of objects; the
  // former are emplace_back'ed, the latter push_back'ed
  ...
  if (fi && (PyTuple_CheckExact(fi) || PyList_CheckExact(fi))) {
    ...
    PyObject* vtype =
        GetAttrDirect((PyObject*)Py_TYPE(vecin), PyStrings::gValueTypePtr);
    bool value_is_vector = false;
    if (vtype && PyLong_Check(vtype)) {
      // if the value_type is a vector, then allow for initialization from
      // sequences
      if (interop::GetTypeAsString(PyLong_AsVoidPtr(vtype))
              .rfind("std::vector", 0) != std::string::npos)
      ...
      for (int i = 0; /* until break */; ++i) {
        ...
        if (item) {
          if (value_is_vector && PySequence_Check(item)) {
            ...
            Py_DECREF(item);
            PyErr_Format(PyExc_TypeError, "argument %d is not a tuple or list",
                         i);
            fill_ok = false;
            break;
          }
    ...
    if (pb_call) {
      for (;;) {
        ...
        if (item) {
          PyObject* pbres =
              PyObject_CallFunctionObjArgs(pb_call, item, nullptr);
          Py_DECREF(item);
          if (!pbres) {
            fill_ok = false;
            break;
...
PyObject* VectorIAdd(PyObject* self, PyObject* args, PyObject* /* kwds */) {
  ...
  // if no getter, it could still be b/c we have a buffer (e.g. numpy); looping
  // over a buffer here is slow, so use insert() instead
  if (PyTuple_GET_SIZE(args) == 1) {
    PyObject* fi = PyTuple_GET_ITEM(args, 0);
    if (PyObject_CheckBuffer(fi) &&
...
PyObject* VectorInit(PyObject* self, PyObject* args, PyObject* /* kwds */) {
  ...
  // implement it directly, except for arrays (which can be passed wholesale)
  // and strings (which won't convert properly as they'll be seen as buffers)
...
static PyObject* vector_iter(PyObject* v) {
  ...
  if (PyUnstable_Object_IsUniqueReferencedTemporary(v) ||
      (((CPPInstance*)v)->fFlags & CPPInstance::kIsValue))
#else
  ...
  PyObject* pyvalue_type =
      PyObject_GetAttr((PyObject*)Py_TYPE(v), PyStrings::gValueTypePtr);
  if (pyvalue_type) {
    PyObject* pyvalue_size =
        GetAttrDirect((PyObject*)Py_TYPE(v), PyStrings::gValueSize);
    if (pyvalue_size) {
    ...
    if (PyLong_Check(pyvalue_type)) {
      interop::TCppType_t value_type = PyLong_AsVoidPtr(pyvalue_type);
      value_type = interop::ResolveType(value_type);
      vi->vi_klass = interop::GetScopeFromType(value_type);
      if (!vi->vi_klass) {
        // look for a special case of pointer to a class type (which is a
        // builtin, but it is more useful to treat it polymorphically by
        // allowing auto-downcasts)
        const std::string& clean_type = TypeManip::clean_type(
            interop::GetTypeAsString(value_type), false, false);
        interop::TCppScope_t c = interop::GetScope(clean_type);
        if (c &&
            TypeManip::compound(interop::GetTypeAsString(value_type)) == "*") {
          vi->vi_klass = c;
          vi->vi_flags = vectoriterobject::kIsPolymorphic;
        }
      }
      if (interop::IsPointerType(value_type))
        vi->vi_flags = vectoriterobject::kIsPolymorphic;
      if (vi->vi_klass) {
        vi->vi_converter = nullptr;
        if (!vi->vi_flags) {
          value_type = interop::ResolveType(value_type);
          if (interop::GetTypeAsString(value_type).back() !=
              '*') // meaning, object stored by-value
            vi->vi_flags = vectoriterobject::kNeedLifeLine;
        }
      } else
      ...
      vi->vi_converter = nullptr;
      if (!vi->vi_stride)
        vi->vi_stride = interop::SizeOf(vi->vi_klass);
      if (!vi->vi_flags)
        vi->vi_flags = vectoriterobject::kNeedLifeLine;
    }
...
static interop::TCppScope_t sVectorBoolTypeID;
...
PyObject* VectorBoolGetItem(CPPInstance* self, PyObject* idx) {
  ...
  if (!CPPInstance_Check(self) || self->ObjectIsA() != sVectorBoolTypeID) {
    PyErr_Format(PyExc_TypeError,
                 ...
                 interop::GetScopedFinalName(self->ObjectIsA()).c_str());
    return nullptr;
  }
  ...
  // get hold of the actual std::vector<bool> (no cast, as vector is never a
  // base)
...
PyObject* MapInit(PyObject* self, PyObject* args, PyObject* /* kwds */) {
  ...
  // PyMapping_Check is not very discriminatory, as it basically only checks for
  // the existence of  __getitem__, hence the most common cases of tuple and
  // list are dropped straight-of-the-bat (the PyMapping_Items call will fail on
  // them).
  if (PyTuple_GET_SIZE(args) == 1 &&
    ...
    // in p3, PyMapping_Items isn't a macro, but a function that short-circuits
    // dict
    ...
    // okay to fall through as long as 'self' has not been created (is done in
    // MapFromPairs)
  }
  ...
  // The given argument wasn't a mapping or tuple of pairs: forward to regular
  // constructor
  PyObject* realInit = PyObject_GetAttr(self, PyStrings::gRealInit);
  if (realInit) {
    PyObject* result = PyObject_Call(realInit, args, nullptr);
...
PyObject* STLContainsWithFind(PyObject* self, PyObject* obj) {
  ...
  if (!result) {
    PyErr_Clear(); // e.g. wrong argument type, which should always lead to
                   // False
    Py_INCREF(Py_False);
    result = Py_False;
  }
...
PyObject* SetInit(PyObject* self, PyObject* args, PyObject* /* kwds */) {
  ...
  if (PyTuple_GET_SIZE(args) == 1 && PySet_Check(PyTuple_GET_ITEM(args, 0))) {
    ...
    if (iter) {
      ...
      while (item) {
        PyObject* insres =
            PyObject_CallFunctionObjArgs(ins_call, item, nullptr);
        Py_DECREF(item);
        if (!insres) {
          Py_DECREF(ins_call);
          Py_DECREF(result);
...
PyObject* STLSequenceIter(PyObject* self) {
  ...
  if (iter) {
    ...
    if (end) {
      if (CPPInstance_Check(iter)) {
        // Guarantee the returned iterator implements Python's iterator
        // protocol. The deferred, name-based __next__ install can miss when an
        // iterator's canonical return-type spelling and its scoped-final-name
        // diverge (e.g libc++'s std::__1::__wrap_iter on macOS) leaving the
        // type without
        // __next__, so iter() rejects it. begin()/end() resolving here proves
        // this is an STL forward iterator, so install the protocol now if it is
        // still absent
        ...
        // use the data member cache to store extra state on the iterator
        // object, without it being visible on the Python side
        ...
        // could store "end" on the object's dictionary anyway, but if end()
        // returns a user-customized object, then its __next__ is probably
        // custom, too
        Py_DECREF(end);
      }
    }
...
{
// Implement a generic python __getitem__ for STL-like classes that are missing
the
// reflection info for their iterators. This is then used for iteration by means
of
...
static inline PyObject* cpyrt_PyString_FromCppString(std::string_view s,
                                                     bool native = true) {
  if (native)
...
#define CPPJIT_IMPL_STRING_PYTHONIZATION(type, name)                           \
  static inline PyObject* name##StringGetData(PyObject* self,                  \
                                              bool native = true) {            \
    if (cpyrt::CPPInstance_Check(self)) {                                      \
      type* obj = ((type*)((CPPInstance*)self)->GetObject());                  \
      if (obj)                                                                 \
        return cpyrt_PyString_FromCppString(*obj, native);                     \
    }                                                                          \
...
#define CPPJIT_IMPL_STRING_PYTHONIZATION_CMP(type, name)                       \
  CPPJIT_IMPL_STRING_PYTHONIZATION(type, name)                                 \
...
CPPJIT_IMPL_STRING_PYTHONIZATION_CMP(std::wstring, STLW)
CPPJIT_IMPL_STRING_PYTHONIZATION_CMP(std::string_view, STLView)
...
PyObject* STLStringReplace(CPPInstance* self, PyObject* args,
  ...
  // both str and std::string have a method "replace", but the Python version
  // only accepts strings and takes no keyword arguments, whereas the C++
...
// both str and std::string have method "find" and "rfin"; try the C++ version
// first and fall back on the Python one in case of failure
// clang-format off
CPYRT_STRING_FINDMETHOD( Find, __cpp_find,  find)
CPYRT_STRING_FINDMETHOD(RFind, __cpp_rfind, rfind)
// clang-format on
...
PyObject* StringViewInit(PyObject* self, PyObject* args, PyObject* /* kwds */) {
  ...
  if (realInit) {
    ...
    PyObject* result =
        PyObject_Call(realInit, newArgs ? newArgs : args, nullptr);
...
PyObject* STLIterNext(PyObject* self) {
  ...
  if (last) {
    ...
    if (!PyObject_RichCompareBool(last, self, Py_EQ)) {
      ...
      if (mustIncrement) {
        // prefer preinc, but allow post-inc; in both cases, it is "self" that
        // has the updated state to dereference
...
#define COMPLEX_METH_GETSET(name, cppname)                                     \
  ...
  PyGetSetDef name##Complex{(char*)#name, (getter)name##ComplexGet,            \
                            (setter)name##ComplexSet, nullptr, nullptr};
...
PyGetSetDef ComplexDReal{(char*)"real", (getter)ComplexDRealGet,
                         (setter)ComplexDRealSet, nullptr, nullptr};
...
static inline bool run_pythonizors(PyObject* pyclass, PyObject* pyname,
...
bool cpyrt::Pythonize(PyObject* pyclass, interop::TCppScope_t scope) {
  const std::string& name = interop::GetScopedFinalName(scope);
  ...
  // the attribute must be a cpyrt overload, otherwise the check gives false
  // positives in the case where the class has a non-function attribute that is
  // called "size".
  if (HasAttrDirect(pyclass, PyStrings::gSize, /*mustBecpyrt=*/true)) {
  ...
  if (!IsTemplatedSTLClass(name, "vector") && // vector is dealt with below
    ...
    if (HasAttrDirect(pyclass, PyStrings::gBegin) &&
        HasAttrDirect(pyclass, PyStrings::gEnd)) {
      // obtain the name of the return type
      const auto& methods =
          interop::GetMethodsFromName(klass->fCppType, "begin");
      if (!methods.empty()) {
        // check return type; if not explicitly an iterator, add it to the
        // "known" return types to add the "next" method on use
        interop::TCppMethod_t meth = methods[0];
        const std::string& resname = interop::GetMethodReturnTypeAsString(meth);
        bool isIterator = gIteratorTypes.find(resname) != gIteratorTypes.end();
        if (!isIterator && interop::GetScope(resname)) {
        ...
        if (isIterator) {
          ...
          // (general class won't work: the return type needs to understand the
          // iterator protocol)
    ...
    if (!((PyTypeObject*)pyclass)->tp_iter && // no iterator resolved
        ...
        PyObject_HasAttr(pyclass, PyStrings::gLen)) {
      // Python will iterate over __getitem__ using integers, but C++ operator[]
      // will never raise a StopIteration. A checked getitem (raising IndexError
  ...
  // operator==/!= are used in op_richcompare of CPPInstance, which subsequently
  // allows comparisons to None; if no operator is available, a hook is
  // installed for lazy lookups in the global and/or class namespace
  if (HasAttrDirect(pyclass, PyStrings::gEq, true) &&
      interop::GetMethodsFromName(klass->fCppType, "__eq__").empty()) {
    PyObject* cppol = PyObject_GetAttr(pyclass, PyStrings::gEq);
    if (!klass->fOperators)
      klass->fOperators = new Utility::PyOperators();
    klass->fOperators->fEq = cppol;
    // re-insert the forwarding __eq__ from the CPPInstance in case there was a
    // Python-side override in the base class
  ...
  if (HasAttrDirect(pyclass, PyStrings::gNe, true) &&
      interop::GetMethodsFromName(klass->fCppType, "__ne__").empty()) {
  ...
  if (interop::IsAggregate(((CPPClass*)pyclass)->fCppType) &&
    ...
    interop::GetDatamembers(kls, datamems);
    if (!datamems.empty()) {
      ...
      arg_types.reserve(ndata);
      arg_names.reserve(ndata);
      arg_defaults.reserve(ndata);
      for (auto data : datamems) {
        if (interop::IsStaticDatamember(data) || !interop::IsPublicData(data))
          continue;

        interop::TCppType_t datammember_type = interop::GetDatamemberType(data);
        const std::string& res =
            interop::IsEnumType(datammember_type)
                ? interop::GetScopedFinalName(
                      interop::GetScopeFromType(datammember_type))
                : interop::GetTypeAsString(
                      interop::ResolveType(datammember_type));
        ...
        if (res.rfind(']') == std::string::npos &&
            res.rfind(')') == std::string::npos) {
          if (!cpd.empty())
            arg_types.push_back(res_clean + cpd);
          else
            arg_types.push_back("const " + res_clean + "&");
          arg_names.push_back(interop::GetFinalName(data));
          if ((!cpd.empty() && cpd.back() == '*') ||
              interop::IsBuiltin(res_clean))
            arg_defaults.push_back("0");
          else {
            interop::TCppScope_t klsid = interop::GetScope(res_clean);
            if (interop::IsDefaultConstructable(klsid))
              arg_defaults.push_back(res_clean + "{}");
          }
        } else {
          codegen_ok =
              false; // TODO: how to support arrays, anonymous enums, etc?
          break;
        }
      }
      ...
      if (codegen_ok && !arg_types.empty()) {
        ...
        for (std::vector<std::string>::size_type i = 0; i < arg_types.size();
             ++i) {
          initdef << ", " << arg_types[i] << " " << arg_names[i];
          if (defaults_ok)
            initdef << " = " << arg_defaults[i];
        }
        initdef << ") {\n  *self = new " << name << "{";
        for (std::vector<std::string>::size_type i = 0; i < arg_names.size();
             ++i) {
          if (i != 0)
            initdef << ", ";
          initdef << arg_names[i];
        ...
        if (interop::Compile(initdef.str(), true /* silent */)) {
          interop::TCppScope_t cis = interop::GetScope("__cppjit_internal");
          const auto& methods =
              interop::GetMethodsFromName(cis, "init_" + rname);
          if (methods.size()) {
  ...
  if (IsTemplatedSTLClass(name, "vector")) {
    ...
    if (!sVectorBoolTypeID)
      sVectorBoolTypeID = interop::GetScope("std::vector<bool>");
    if (klass->fCppType == sVectorBoolTypeID) {
      Utility::AddToClass(pyclass, "__getitem__",
                          (PyCFunction)VectorBoolGetItem, METH_O);
      Utility::AddToClass(pyclass, "__setitem__",
                          (PyCFunction)VectorBoolSetItem);
    } else {
      ...
      if (vtype) { // actually resolved?
        ...
        pyvalue_type =
            PyUnicode_FromString(interop::GetTypeAsString(vtype).c_str());
        PyObject_SetAttr(pyclass, PyStrings::gValueType, pyvalue_type);
        Py_DECREF(pyvalue_type);
      }

      size_t typesz = interop::SizeOfType(vtype);
      if (typesz) {
    ...
    // reflection does not always expose it, so fall back to a find()-based
    // __contains__ when none was reflected (still O(log n)/O(1), never
    // iterating).
    if (!HasAttrDirect(pyclass, PyStrings::gContains))
      Utility::AddToClass(pyclass, "__contains__",
                          (PyCFunction)STLContainsWithFind, METH_O);
  }
    ...
    // reflection does not always expose it, so fall back to a find()-based
    // __contains__ when none was reflected (still O(log n), never iterating).
    if (!HasAttrDirect(pyclass, PyStrings::gContains))
      Utility::AddToClass(pyclass, "__contains__",
                          (PyCFunction)STLContainsWithFind, METH_O);
  }
    ...
    Utility::AddToClass(pyclass, "__getitem__", (PyCFunction)PairUnpack,
                        METH_O);
    Utility::AddToClass(pyclass, "__len__", (PyCFunction)ReturnTwo,
                        METH_NOARGS);
  }
  ...
  if (IsTemplatedSTLClass(name, "shared_ptr") ||
    ...
    Utility::AddToClass(pyclass, "__eq__", (PyCFunction)STLStringIsEqual,
                        METH_O);
    Utility::AddToClass(pyclass, "__ne__", (PyCFunction)STLStringIsNotEqual,
                        METH_O);
    // Python's `in` operator needs __contains__ on the proxy regardless of the
    // C++ standard cpyrt is built with; it never dispatches to C++'s
    ...
    Utility::AddToClass(pyclass, "__cmp__", (PyCFunction)STLViewStringCompare,
                        METH_O);
    Utility::AddToClass(pyclass, "__eq__", (PyCFunction)STLViewStringIsEqual,
                        METH_O);
    ...
    Utility::AddToClass(pyclass, "__cmp__", (PyCFunction)STLWStringCompare,
                        METH_O);
    Utility::AddToClass(pyclass, "__eq__", (PyCFunction)STLWStringIsEqual,
                        METH_O);
    Utility::AddToClass(pyclass, "__ne__", (PyCFunction)STLWStringIsNotEqual,
                        METH_O);
  }
    ...
    PyObject_SetAttrString(
        pyclass, "real",
        PyDescr_NewGetSet((PyTypeObject*)pyclass, &ComplexDReal));
    ...
    PyObject_SetAttrString(
        pyclass, "real",
        PyDescr_NewGetSet((PyTypeObject*)pyclass, &realComplex));
    Utility::AddToClass(pyclass, "__cpp_imag", "imag");
    PyObject_SetAttrString(
        pyclass, "imag",
  ...
  //   - explicit pythonization: won't fall through to the base classes and is
  ...
  // call registered pythonizors, if any: first run the namespace-specific
  // pythonizors, then the global ones (the idea is to allow writing a
  // pythonizor that see all classes)
  bool pstatus = true;
