...
cpyrt::PyCallArgs::~PyCallArgs() {
  ...
  if (fFlags & kDoItemDecref) {
    for (Py_ssize_t iarg = 0; iarg < cpyrt_PyArgs_GET_SIZE(fArgs, fNArgsf);
         ++iarg)
...
//- private helpers ----------------------------------------------------------
inline bool cpyrt::CPPMethod::VerifyArgCount_(Py_ssize_t actual) {
...
//----------------------------------------------------------------------------
inline void cpyrt::CPPMethod::Copy_(const CPPMethod& /* other */) {
...
//----------------------------------------------------------------------------
inline void cpyrt::CPPMethod::Destroy_() {
  // destroy executor and argument converters
  if (fExecutor && fExecutor->HasState())
    delete fExecutor;
  fExecutor = nullptr;
...
//----------------------------------------------------------------------------
inline PyObject* cpyrt::CPPMethod::ExecuteFast(void* self, ptrdiff_t offset,
                                               CallContext* ctxt) {
  // call into C++ through fExecutor; abstracted out from Execute() to prevent
  // some code duplication with ProtectedCall()
  PyObject* result = nullptr;

  try { // C++ try block
    result = fExecutor->Execute(
        fMethod, interop::TCppObject_t((void*)((intptr_t)self + offset)), ctxt);
  } catch (PyException&) {
    ctxt->fFlags |= CallContext::kPyException;
    result = nullptr; // error already set
  } catch (std::exception& e) {
    // attempt to set the exception to the actual type, to allow catching with
    // the Python C++ type
    static interop::TCppScope_t exc_type =
        interop::GetFullScope("std::exception");
    ...
    // TODO: factor this code with the same in ProxyWrappers (and cache it there
    // to be able to look up based on TCppType_t):
    interop::TCppScope_t actual = interop::GetActualClass(exc_type, &e);
    const std::string& finalname = interop::GetScopedFinalName(actual);
    const std::string& parentname = TypeManip::extract_namespace(finalname);
    PyObject* parent = CreateScopeProxy(parentname);
    if (parent) {
      pyexc_type = PyObject_GetAttrString(
          parent,
          parentname.empty()
              ? finalname.c_str()
              : finalname.substr(parentname.size() + 2, std::string::npos)
                    .c_str());
      Py_DECREF(parent);
    }

    if (pyexc_type) {
      // create a copy of the exception (TODO: factor this code with the same in
      // ProxyWrappers)
      PyObject* pyclass = cpyrt::GetScopeProxy(actual);
      PyObject* source = BindCppObjectNoCast(&e, actual);
      PyObject* pyexc_copy =
          PyObject_CallFunctionObjArgs(pyclass, source, nullptr);
      Py_DECREF(source);
      Py_DECREF(pyclass);
      if (pyexc_copy) {
        pyexc_obj = CPPExcInstance_Type.tp_new((PyTypeObject*)pyexc_type,
                                               nullptr, nullptr);
...
// TODO: covers the PyException throw case, which does not seem to work on
...
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::InitExecutor_(Executor*& executor,
                                     CallContext* /* ctxt */) {
  // install executor conform to the return type
  executor = (bool)fMethod == true
                 ? CreateExecutor(interop::GetMethodReturnType(fMethod))
                 : CreateExecutor(interop::GetScopedFinalName(fScope));
...
//----------------------------------------------------------------------------
std::string cpyrt::CPPMethod::GetSignatureString(bool fa) {
  // built a signature representation (used for doc strings)
  return interop::GetMethodSignature(fMethod, fa);
}

//----------------------------------------------------------------------------
void cpyrt::CPPMethod::SetPyError_(PyObject* msg) {
  ...
  const bool isCppExc =
      evalue && PyType_IsSubtype((PyTypeObject*)etype, &CPPExcInstance_Type);
  std::string details;
...
extern std::unordered_map<interop::TCppType_t, interop::TCppType_t>
    TypeReductionMap;
...
//----------------------------------------------------------------------------
cpyrt::CPPMethod::CPPMethod(const CPPMethod& other)
    : PyCallable(other), fMethod(other.fMethod), fScope(other.fScope) {
  Copy_(other);
}

//----------------------------------------------------------------------------
cpyrt::CPPMethod& cpyrt::CPPMethod::operator=(const CPPMethod& other) {
  if (this != &other) {
    Destroy_();
    Copy_(other);
    fScope = other.fScope;
...
//----------------------------------------------------------------------------
cpyrt::CPPMethod::~CPPMethod() { Destroy_(); }
 ...
 * @return PyObject* A Python string with the full method prototype, namespaces
 * included.
 *
 * For example, given:
 *
 ...
 * 'int a::b::c::foo(int x)'
 */
PyObject* cpyrt::CPPMethod::GetPrototype(bool fa) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::Reflex(interop::Reflex::RequestId_t request,
                                   interop::Reflex::FormatId_t format) {
  // C++ reflection tooling for methods.

  if (request == interop::Reflex::RETURN_TYPE) {
...
//----------------------------------------------------------------------------
int cpyrt::CPPMethod::GetPriority() {
  // To help with overload selection, methods are given a priority based on the
  // affinity of Python and C++ types. Priority only matters for methods that
  // have an equal number of arguments and types that are possible substitutes
  // (the normal selection mechanisms would simply distinguish them otherwise).
  ...
  //  const char* >> char
  //
  // Further, all integer types are preferred over floating point b/c int to
  // float is allowed implicitly, float to int is not.
  //
  // Special cases that are disliked include void* and unknown/incomplete types.
  // Also, moves are preferred over references. std::initializer_list is not a
  // nice conversion candidate either, but needs to be higher priority to mix
  // well with implicit conversions.
  ...
  const size_t nArgs = interop::GetMethodNumArgs(fMethod);
  for (int iarg = 0; iarg < (int)nArgs; ++iarg) {
    const std::string aname = interop::GetMethodArgTypeAsString(fMethod, iarg);
    ...
    if (compact.find("void*") != std::string::npos) {
      ...
      // There's a bit of hysteresis here for templates: once GetScope() is
      // called, their IsComplete() succeeds, the other way around it does not.
      // Since GetPriority() is likely called several times in a sort, the
      // GetScope() _must_ come first, or different GetPriority() calls may
      // return different results (since the 2nd time, GetScope() will have been
      // called from the first), killing the stable_sort.
      ...
      } else if (scope && !interop::IsComplete(scope)) {
  ...
  // prefer methods w/o optional arguments b/c ones with optional arguments are
  // easier to select by providing the optional arguments explicitly
  priority += ((int)interop::GetMethodReqArgs(fMethod) - (int)nArgs);

  // add a small penalty to prefer non-const methods over const ones for
  // get/setitem
  if (interop::IsConstMethod(fMethod) &&
      interop::GetName(interop::TCppScope_t(fMethod.data)) == "operator[]")
    priority += -10;

  // constructors are prefered
  if (interop::IsConstructor(fMethod))
    priority += 100;
...
bool cpyrt::CPPMethod::IsGreedy() {
  ...
  const size_t nArgs = interop::GetMethodReqArgs(fMethod);
  if (!nArgs)
    return false;
...
//----------------------------------------------------------------------------
int cpyrt::CPPMethod::GetMaxArgs() {
  return (int)interop::GetMethodNumArgs(fMethod);
}

//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::GetCoVarNames() {
...
PyObject* cpyrt::CPPMethod::GetArgDefault(int iarg, bool silent) {
  // get and evaluate the default value (if any) of argument iarg of this method
  if (iarg >= (int)GetMaxArgs())
    return nullptr;

  // borrowed reference to cppjit.gbl module to use its dictionary to eval in
  static PyObject* gbl =
      PyDict_GetItemString(PySys_GetObject((char*)"modules"), "cppjit.gbl");

  std::string defvalue = interop::GetMethodArgDefault(fMethod, iarg);
  if (!defvalue.empty()) {
    ...
    if (defvalue.rfind('(') != std::string::npos) { // constructor-style call
      // try to tickle scope creation, just in case, first look in the scope
      // where the function lives, then in the global scope
      std::string possible_scope = defvalue.substr(0, defvalue.rfind('('));
      if (!interop::IsBuiltin(possible_scope)) {
        std::string cand_scope =
            interop::GetScopedFinalName(fScope) + "::" + possible_scope;
        ...
        if (!scope) {
          ...
          defvalue = cand_scope +
                     defvalue.substr(defvalue.rfind('('), std::string::npos);
        }
      }
    }
    ...
    if (!scope) {
      // a couple of common cases that python doesn't like (technically, 'L' is
      // okay with older pythons, but C long will always fit in Python int, so
      // no need to bother)
    ...
    // attempt to evaluate the string representation (compilation is first to
    // code to allow the error message to indicate where it's coming from)
    PyObject* pyval = nullptr;

    PyObject* pycode = Py_CompileString(
        (char*)defvalue.c_str(), "cppjit_default_compiler", Py_eval_input);
    if (pycode) {
  ...
  PyErr_Format(PyExc_TypeError, "Could not construct default value for: %s",
               interop::GetMethodArgName(fMethod, iarg).c_str());
  return nullptr;
}
...
interop::AllocType cpyrt::CPPMethod::GetAllocBehaviour() {
  if (fAllocType.has_value())
    return *fAllocType;
  interop::TCppMethod_t method = GetMethod();
  if (!method) {
    fAllocType = interop::AllocType::None;
    return interop::AllocType::None;
  }
  if (!IsPointerType(interop::GetMethodReturnType(method))) {
    fAllocType = interop::AllocType::None;
    return interop::AllocType::None;
  }
  interop::AllocType attrResult = interop::IsAllocator(method);
  fAllocType = attrResult;
  return attrResult;
}
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::GetDeallocBehaviour() {
  if (fDeallocType.has_value())
    return *fDeallocType;
  interop::TCppMethod_t method = GetMethod();
  if (!method) {
    fDeallocType = false;
  ...
  bool attrResult = interop::IsDeallocator(GetMethod());
  fDeallocType = attrResult;
  return attrResult;
}
//----------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
interop::TCppFuncAddr_t cpyrt::CPPMethod::GetFunctionAddress() {
  // Return the C++ pointer of this function
  return interop::GetFunctionAddress(fMethod,
                                     false /* don't check fast path envar */);
}

//----------------------------------------------------------------------------
int cpyrt::CPPMethod::GetArgMatchScore(PyObject* args_tuple) {
  Py_ssize_t n = PyTuple_Size(args_tuple);

  int req_args = interop::GetMethodReqArgs(fMethod);
...
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::Initialize(CallContext* ctxt) {
  ...
  fArgsRequired =
      (int)((bool)fMethod == true ? interop::GetMethodReqArgs(fMethod) : 0);
...
bool cpyrt::CPPMethod::ProcessKwds(PyObject* self_in, PyCallArgs& cargs) {
  ...
  if (!fArgIndices) {
    ...
    for (int iarg = 0; iarg < (int)interop::GetMethodNumArgs(fMethod); ++iarg)
      (*fArgIndices)[interop::GetMethodArgName(fMethod, iarg)] = iarg;
  }

  Py_ssize_t nArgs =
      cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf) + (self_in ? 1 : 0);
  ...
  Py_ssize_t npos_args = cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf);
  for (Py_ssize_t ikey = 0; ikey < nKeys; ++ikey) {
    ...
    vArgs[p->second] =
        value; // no INCREF yet for simple cleanup in case of error
  }

  // if maxpos < nArgs, it will be detected & reported as a duplicate below
  Py_ssize_t maxargs = maxpos + 1;
  cpyrt_PyArgs_t newArgs = cpyrt_PyArgs_New(maxargs);

  // set all values to zero to be able to check them later (this also guarantees
  // normal cleanup by the tuple deallocation)
  for (Py_ssize_t i = 0; i < maxargs; ++i)
    cpyrt_PyArgs_SET_ITEM(newArgs, i, nullptr);

  // fill out the positional arguments
  Py_ssize_t start = 0;
  if (self_in) {
    Py_INCREF(self_in);
    cpyrt_PyArgs_SET_ITEM(newArgs, 0, self_in);
    start = 1;
  }
  ...
  for (Py_ssize_t i = start; i < nArgs; ++i) {
    if (vArgs[i]) {
      SetPyError_(cpyrt_PyText_FromFormat(
          ...
          (int)i + 1));
      cpyrt_PyArgs_DEL(newArgs);
      return false;
    }

    PyObject* item = cpyrt_PyArgs_GET_ITEM(cargs.fArgs, i);
    Py_INCREF(item);
    cpyrt_PyArgs_SET_ITEM(newArgs, i, item);
  }
...
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::ProcessArgs(PyCallArgs& cargs) {
  ...
  if (cpyrt_PyArgs_GET_SIZE(cargs.fArgs, cargs.fNArgsf) != 0) {
    CPPInstance* pyobj = (CPPInstance*)cpyrt_PyArgs_GET_ITEM(cargs.fArgs, 0);

    // demand cpyrt object, and an argument that may match down the road
    if (CPPInstance_Check(pyobj)) {
      interop::TCppScope_t oisa = pyobj->ObjectIsA();
      if (fScope == interop::GetGlobalScope() || // free global
          oisa == nullptr ||                     // null pointer or ctor call
          oisa == fScope ||                      // matching types
          interop::IsSubclass(oisa, fScope)) {   // id.
...
//----------------------------------------------------------------------------
bool cpyrt::CPPMethod::ConvertAndSetArgs(cpyrt_PyArgs_t args, size_t nargsf,
                                         CallContext* ctxt) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CPPMethod::Call(CPPInstance*& self, cpyrt_PyArgs_t args,
  ...
  if (fArgsRequired || cpyrt_PyArgs_GET_SIZE(args, cargs.fNArgsf)) {
  ...
  // get its class
  interop::TCppScope_t derived = self->ObjectIsA();
  ...
  if (CPPInstance_Check(pyobj) && derived && pyobj->ObjectIsA() == derived &&
 ...
 * ('a', 'b', 'c')
 */
PyObject* cpyrt::CPPMethod::GetSignatureNames() {
 ...
 */
PyObject* cpyrt::CPPMethod::GetSignatureTypes() {
...
//----------------------------------------------------------------------------
std::string cpyrt::CPPMethod::GetReturnTypeName() {
  return interop::GetMethodReturnTypeAsString(fMethod);
}
