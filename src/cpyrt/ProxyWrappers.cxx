...
extern std::unordered_set<interop::TCppScope_t> gPinnedTypes;
} // namespace cppjit::cpyrt

// to prevent having to walk scopes, track python classes by C++ class
typedef std::unordered_map<interop::TCppScope_t, PyObject*> PyClassMap_t;
...
static PyObject* CreateNewCppProxyClass(interop::TCppScope_t klass,
                                        PyObject* pybases) {
  // Create a new python shadow class with the required hierarchy and
  // meta-classes.
  ...
  std::string name = interop::GetFinalName(klass);

  // create meta-class, add a dummy __module__ to pre-empt the default setting
  PyObject* args =
      Py_BuildValue((char*)"sO{}", (name + "_meta").c_str(), pymetabases);
...
static inline void AddPropertyToClass(PyObject* pyclass,
  ...
  if (PyErr_Occurred()) {
    ...
    // 3. data is defined in both parent and base class (using
    // parent::attribute;)
    PyErr_Clear();
  }

  // allow access at the class level (always add after setting instance level)
  if (interop::IsStaticDatamember(data) || interop::IsEnumConstant(data))
    PyType_Type.tp_setattro((PyObject*)Py_TYPE(pyclass), pname,
                            (PyObject*)property);
...
static inline void AddScopeToParent(PyObject* parent, const std::string& name,
  ...
  if (CPPScope_Check(parent))
    ...
    PyObject_SetAttr(parent, pyname, newscope);
  Py_DECREF(pyname);
}

static inline PyObject* GetAttrDirect(PyObject* pyclass, PyObject* pyname) {
...
static inline void sync_templates(PyObject* pyclass,
...
static int BuildScopeProxyDict(interop::TCppScope_t scope, PyObject* pyclass,
                               const unsigned int flags) {
  // Collect methods and data for the given scope, and add them to the given
  // python proxy object.

  // some properties that'll affect building the dictionary
  bool isNamespace = interop::IsNamespace(scope);
  bool isComplete = interop::IsComplete(scope);
  bool hasConstructor = false;
  interop::TCppMethod_t potGetItem = nullptr;
  ...
  if (isComplete)
    interop::GetClassMethods(scope, methods);
  ...
  for (auto& method : methods) {
    ...
    std::string mtCppName = interop::GetName(interop::TCppScope_t(method.data));

    // special case trackers
    bool setupSetItem = false;
    bool isConstructor = interop::IsConstructor(method);

    // Here isTemplate means to ignore templated constructors, that is handled
    // in the next loop.
    bool isTemplate = interop::IsTemplatedMethod(method) && !isConstructor;
    ...
    std::string mtName = Utility::MapOperatorName(
        mtCppName, interop::GetMethodNumArgs(method), &isStubbedOperator);
    if (mtName.empty())
      continue;
    ...
    if (isCall || mtName == "__getitem__") {
      const std::string& qual_return =
          interop::GetMethodReturnTypeAsString(method);
      const std::string& cpd = TypeManip::compound(qual_return);
      if (!cpd.empty() && cpd[cpd.size() - 1] == '&' &&
          qual_return.find("const", 0, 5) == std::string::npos) {
        if (isCall && !potGetItem)
          potGetItem = method;
        setupSetItem = true; // will add methods as overloads
      } else if (isCall && 1 < interop::GetMethodNumArgs(method)) {
        // not a non-const by-ref return, thus better __getitem__ candidate; the
        // requirement for multiple arguments is that there is otherwise no
        // benefit over the use of normal __getitem__ (this allows
        // multi-indexing arguments, which is clean in Python, but not allowed
        // in C++)
        potGetItem = method;
      }
    }
    ...
    if (storeOnTemplate) {
      ...
      // continue processing to actually add the method so that the proxy can
      // find it on the class when called explicitly
    }

    // construct the holder
    PyCallable* pycall = nullptr;
    if (interop::IsStaticMethod(method)) // class method
    ...
    if (storeOnTemplate) {
      ...
      if (setupSetItem) {
        TemplateProxy* pysi =
            (TemplateProxy*)GetAttrDirect(pyclass, PyStrings::gSetItem);
        if (!TemplateProxy_Check(pysi)) {
          CPPOverload* precursor =
              (CPPOverload_Check(pysi)) ? (CPPOverload*)pysi : nullptr;
          if (pysi && !precursor)
            Py_DECREF(pysi); // something unknown, just drop it
          pysi = TemplateProxy_New(mtCppName, "__setitem__", pyclass);
          if (precursor)
            pysi->MergeOverload(precursor);
      ...
      Callables_t& md =
          (*(cache.insert(std::make_pair(mtName, Callables_t())).first)).second;
      md.push_back(pycall);

      // special case for operator[]/() that returns by ref, use for
      // getitem/call and setitem
      if (setupSetItem) {
        Callables_t& setitem =
            (*(cache
                   .insert(std::make_pair(std::string("__setitem__"),
                                          Callables_t()))
                   .first))
                .second;
  ...
  std::vector<interop::TCppMethod_t> templ_methods;
  interop::GetTemplatedMethods(scope, templ_methods);
  for (auto& method : templ_methods) {
    const std::string mtCppName =
        interop::GetName(interop::TCppScope_t(method.data));
    // the number of arguments isn't known until instantiation and as far as C++
    // is concerned, all same-named operators are simply overloads; so will
    // pre-emptively add both names if with and without arguments differ,
    // letting the normal overload mechanism resolve on call
    bool isConstructor = interop::IsConstructor(method);

    // first add with no arguments
    std::string mtName0 =
        isConstructor ? "__init__" : Utility::MapOperatorName(mtCppName, false);
  ...
  if (!hasConstructor) {
    ...
    if (!isComplete) {
      ...
      defctor =
          new CPPIncompleteClassConstructor(scope, interop::TCppMethod_t{});
    } else if (interop::IsAbstract(scope)) {
      defctor = new CPPAbstractClassConstructor(scope, interop::TCppMethod_t{});
    } else if (isNamespace) {
      defctor = new CPPNamespaceConstructor(scope, interop::TCppMethod_t{});
    } else {
      defctor =
          new CPPAllPrivateClassConstructor(scope, interop::TCppMethod_t{});
    }
  ...
  if (potGetItem) {
    Callables_t& getitem =
        (*(cache
               .insert(
                   std::make_pair(std::string("__getitem__"), Callables_t()))
               .first))
            .second;
  ...
  PyObject* dct = PyObject_GetAttr(pyclass, PyStrings::gDict);
  for (CallableCache_t::iterator imd = cache.begin(); imd != cache.end();
       ++imd) {
    // in order to prevent removing templated editions of this method (which
    // were set earlier, above, as a different proxy object), we'll check and
  ...
  std::vector<interop::TCppScope_t> datamembers;
  interop::GetDatamembers(scope, datamembers);
  for (auto& datamember : datamembers) {
    // allow only public members
    if (!interop::IsPublicData(datamember))
      continue;

    // enum datamembers (this in conjunction with previously collected enums
    // above)
    if (interop::IsEnumType(interop::GetDatamemberType(datamember)) &&
        interop::IsEnumConstant(datamember)) {
      // two options: this is a static variable, or it is the enum value, the
      // latter already exists, so check for it and move on if set
      PyObject* eset = PyObject_GetAttrString(
          pyclass,
          const_cast<char*>(interop::GetFinalName(datamember).c_str()));
      if (eset) {
        Py_DECREF(eset);
        continue;
      }
      ...
      // it could still be that this is an anonymous enum, which is not in the
      // list provided by the class
      if (strstr(interop::GetDatamemberTypeAsString(datamember).c_str(),
                 "(anonymous)") != 0 ||
          strstr(interop::GetDatamemberTypeAsString(datamember).c_str(),
                 "(unnamed)") != 0) {
...
//----------------------------------------------------------------------------
static void CollectUniqueBases(interop::TCppScope_t klass,
                               std::deque<interop::TCppScope_t>& uqb) {
  // collect bases in acceptable mro order, while removing duplicates (this may
  // break the overload resolution in esoteric cases, but otherwise the class
  // can not be used at all, as CPython will refuse the mro).
  size_t nbases = interop::GetNumBases(klass);

  for (size_t ibase = 0; ibase < nbases; ++ibase) {
    interop::TCppScope_t tp = interop::GetBaseScope(klass, ibase);
    int decision = 2;
    if (!tp)
      continue; // means this base with not be available Python-side
    for (size_t ibase2 = 0; ibase2 < uqb.size(); ++ibase2) {
      ...
      if (interop::IsSubclass(tp, uqb[ibase2])) {
...
static PyObject* BuildCppClassBases(interop::TCppScope_t klass) {
  // Build a tuple of python proxy classes of all the bases of the given
  // 'klass'.
  std::deque<interop::TCppScope_t> uqb;
  ...
  if (nbases == 0) {
    ...
    for (std::deque<interop::TCppScope_t>::size_type ibase = 0; ibase < nbases;
         ++ibase) {
    ...
    if (!PyObject_IsSubclass(PyTuple_GET_ITEM(pybases, 0),
                             (PyObject*)&CPPInstance_Type)) {
...
namespace cppjit::cpyrt {
PyObject* CppType_To_PyObject(interop::TCppType_t type, std::string name,
                              interop::TCppScope_t parent_scope,
                              PyObject* parent) {
  interop::TCppType_t resolved_type = interop::ResolveType(type);
  if (gPyTypeMap) {
    const std::string& resolved = interop::GetTypeAsString(resolved_type);
    PyObject* tc =
        PyDict_GetItemString(gPyTypeMap, resolved.c_str()); // borrowed
    if (tc && PyCallable_Check(tc)) {
      const std::string& scName = interop::GetScopedFinalName(parent_scope);
      PyObject* nt =
          PyObject_CallFunction(tc, (char*)"ss", name.c_str(),
                                scName != "<unnamed>" ? scName.c_str() : "");
      if (nt) {
        if (parent) {
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateScopeProxy(const std::string& name, PyObject* parent,
  ...
  if (parent) {
    if (CPPScope_Check(parent))
      ...
      if (!parname) {
        PyErr_Format(PyExc_SystemError, "given scope has no name for %s",
                     name.c_str());
        return nullptr;
      }
      ...
      if (PyErr_Occurred())
        return nullptr;
      parent_scope = interop::GetScope(scName);
    }
  ...
  if (name == "") {
    interop::TCppScope_t klass = interop::GetGlobalScope();
    Py_INCREF(gThisModule);
    parent = gThisModule;
    return CreateScopeProxy(klass, parent, flags);
  } else if (interop::TCppScope_t klass =
                 interop::GetScope(name, parent_scope)) {
    if (interop::IsTypedefed(klass) &&
        interop::IsPointerType(interop::GetTypeFromScope(klass)) &&
        interop::IsClass(interop::GetUnderlyingScope(klass)))
      return nullptr; // this is handled by the caller; typedef to class pointer
    return CreateScopeProxy(klass, parent, flags);
  } else if (interop::IsBuiltin(name)) {
    interop::TCppType_t type = interop::GetType(name);
  ...
  PyErr_Format(PyExc_TypeError, "\'%s\' is not a known C++ class",
               name.c_str());
  Py_XDECREF(parent);
  return nullptr;
}
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateScopeProxy(interop::TCppScope_t scope, PyObject* parent,
                                  const unsigned flags) {
  // Convenience function with a lookup first through the known existing
  // proxies.
  PyObject* pyclass = GetScopeProxy(scope);
  if (pyclass)
    return pyclass;

  interop::TCppScope_t parent_scope = nullptr;
  if (CPPScope_Check(parent))
    parent_scope = ((CPPScope*)parent)->fCppType;
  if (!parent_scope)
    parent_scope = interop::GetParentScope(scope);
  ...
  std::string name = interop::GetFinalName(scope);
  bool is_typedef = false;
  std::string typedefed_name = "";
  if (interop::IsTypedefed(scope)) {
    is_typedef = true;
    typedefed_name = interop::GetFullName(scope);
    interop::TCppScope_t underlying_scope = interop::GetUnderlyingScope(scope);
    if ((underlying_scope) && (underlying_scope != scope)) {
      scope = underlying_scope;
    } else {
      if (PyObject* result = CppType_To_PyObject(
              interop::GetTypeFromScope(scope), name, parent_scope, parent))
        return result;

      PyErr_Format(PyExc_TypeError, "\'%s\' is not a known C++ class",
                   interop::GetScopedFinalName(scope).c_str());
      Py_XDECREF(parent);
      return nullptr;
    }
  }

  if (interop::IsTemplate(scope)) {
    // a "naked" templated class is requested: return callable proxy for
    // instantiations
    ...
    PyObject* pytemplate = PyObject_CallFunction(
        ...
        const_cast<char*>(interop::GetScopedFinalName(scope).c_str()),
        cppscope);
    Py_DECREF(pytcl);
  ...
  // if the scope was earlier found as actual, then we're done already,
  // otherwise build a new scope proxy
  if (!pyscope) {
    ...
    // store a ref from cppjit scope id to new python class
  ...
  if (pyscope && !(((CPPScope*)pyscope)->fFlags & CPPScope::kIsInComplete)) {
    ...
    if (interop::IsTemplateInstantiation(scope))
      name = interop::GetScopedFinalName(scope);
...
//----------------------------------------------------------------------------
PyObject* cpyrt::CreateExcScopeProxy(PyObject* pyscope, PyObject* pyname,
                                     PyObject* parent) {
  // To allow use of C++ exceptions in lieue of Python exceptions, they need to
  // derive from BaseException, which can not mix with the normal CPPInstance
  // and use of the meta-class. Instead, encapsulate them in a forwarding class
  // that derives from Pythons Exception class
  ...
  // for deriving from BaseException is to guarantee the layout needed for
  // storing traces. If some other base is std::exception (as e.g.
  // boost::bad_any_cast) or also derives from std::exception, then there are
  // two trace locations. OTOH, if the other class is a non-exception type, then
  // the exception class does not need to derive from it because it can never be
  // caught as that type forwarding to the proxy will work as expected, through,
  ...
  if (nbases == 0) {
    ...
    for (std::deque<interop::TCppScope_t>::size_type ibase = 0; ibase < nbases;
      ...
      interop::TCppScope_t parent_scope = interop::GetParentScope(uqb[ibase]);
      ...
      PyObject* excbase = PyObject_GetAttrString(
          base_parent, interop::GetFinalName(uqb[ibase]).c_str());
      ...
      if (PyType_IsSubtype((PyTypeObject*)excbase, &CPPExcInstance_Type)) {
        ...
        if (interop::GetScopedFinalName(uqb[ibase]) != "std::exception")
          break;
      } else {
  ...
  // meta-class attributes (__cpp_name__, etc.) can not be resolved lazily so
  // add them directly instead in case they are needed
  PyObject* dct = PyTuple_GET_ITEM(args, 2);
  PyDict_SetItem(dct, PyStrings::gUnderlying, pyscope);
  PyDict_SetItem(dct, PyStrings::gName,
                 PyObject_GetAttr(pyscope, PyStrings::gName));
  PyDict_SetItem(dct, PyStrings::gCppName,
                 PyObject_GetAttr(pyscope, PyStrings::gCppName));
  PyDict_SetItem(dct, PyStrings::gModule,
                 PyObject_GetAttr(pyscope, PyStrings::gModule));
...
//----------------------------------------------------------------------------
PyObject* cpyrt::BindCppObjectNoCast(interop::TCppObject_t address,
  ...
  void* r_address =
      isRef ? (address ? *(void**)address.data : nullptr) : address.data;

  // check whether the object to be bound is a smart pointer that needs
  // embedding
  PyObject* smart_type = (!(flags & CPPInstance::kNoWrapConv) &&
                          (((CPPClass*)pyclass)->fFlags & CPPScope::kIsSmart))
                             ? pyclass
                             : nullptr;
  if (smart_type) {
    interop::TCppScope_t underlying =
        ((CPPSmartClass*)smart_type)->fUnderlyingType;
    ...
    if (address && !isRef) {
      if (void* deref =
              interop::CallR(((CPPSmartClass*)smart_type)->fDereferencer,
                             address, 0, nullptr)) {
        interop::TCppScope_t clActual =
            interop::GetActualClass(underlying, deref);
        if (clActual && clActual != underlying &&
            interop::IsSubclass(clActual, underlying) &&
            interop::GetBaseOffset(clActual, underlying, deref,
                                   -1 /* down-cast */) == 0)
          underlying = clActual;
      }
    }
  ...
  // TODO: make sure that a consistent address is used (may have to be done in
  // BindCppObject)
  if (address && !(flags & CPPInstance::kIsValue) /* always fresh */ &&
      !noReg) {
    PyObject* oldPyObject =
        MemoryRegulator::RetrievePyObject(r_address, pyclass);

    // embedded smart pointers are registered with the class of the underlying
    // type, b/c there is no Python proxy for the smart pointer itself in that
    // case; check whether the result found matches the smart-ness and if so,
    // whether the smart pointer found is the same as the one requested (note:
    ...
    if (oldPyObject) {
      ...
      if ((bool)smart_type == o_pyobj->IsSmart() &&
          r_address ==
              (smart_type ? o_pyobj->GetObjectRaw() : o_pyobj->GetObject()) &&
  ...
  CPPInstance* pyobj = (CPPInstance*)((PyTypeObject*)pyclass)
                           ->tp_new((PyTypeObject*)pyclass, args, nullptr);
  Py_DECREF(args);

  // bind, register and return if successful
  if (pyobj != 0) { // fill proxy value?
    unsigned objflags =
        flags & (CPPInstance::kIsReference | CPPInstance::kIsPtrPtr |
                 CPPInstance::kIsValue | CPPInstance::kIsOwner |
                 CPPInstance::kIsActual);
    pyobj->Set(address.data, (CPPInstance::EFlags)objflags);

    if (smart_type)
      pyobj->SetSmart(smart_type);

    // do not register null pointers, references (regulated objects can be
    // referenced, but referenced objects can not be regulated), or direct usage
    // of smart pointers or iterators
    if (address && !isRef && !noReg)
      MemoryRegulator::RegisterPyObject(
          pyobj, smart_type ? pyobj->GetObjectRaw() : pyobj->GetObject());
  }

  // successful completion; wrap exception options to make them raiseable,
  // normal return otherwise
  if (((CPPClass*)pyclass)->fFlags & CPPScope::kIsException) {
    PyObject* exc_obj =
        CPPExcInstance_Type.tp_new(&CPPExcInstance_Type, nullptr, nullptr);
...
//----------------------------------------------------------------------------
PyObject* cpyrt::BindCppObject(interop::TCppObject_t address,
                               interop::TCppScope_t klass,
                               const unsigned flags) {
  // if the object is a null pointer, return a typed one (as needed for
  // overloading)
  if (!address)
  ...
  if (!isRef && (gPinnedTypes.empty() ||
                 gPinnedTypes.find(klass) == gPinnedTypes.end())) {
    interop::TCppScope_t clActual = interop::GetActualClass(klass, address);

    if (clActual) {
      if (clActual != klass) {
        intptr_t offset =
            interop::GetBaseOffset(clActual, klass, address, -1 /* down-cast */,
                                   true /* report errors */);
...
//----------------------------------------------------------------------------
PyObject* cpyrt::BindCppObjectArray(interop::TCppObject_t address,
