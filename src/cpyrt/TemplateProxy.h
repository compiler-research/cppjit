#ifndef CPYRT_TEMPLATEPROXY_H
#define CPYRT_TEMPLATEPROXY_H
...
typedef std::unordered_map<std::string, std::vector<TP_DispatchEntry_t>>
    TP_DispatchMap_t;
...
public:
  ...
  PyObject* Instantiate(const std::string& fname, cpyrt_PyArgs_t tmplArgs,
...
template <typename T> inline bool TemplateProxy_Check(T* object) {
  return object && PyObject_TypeCheck(object, &TemplateProxy_Type);
}

template <typename T> inline bool TemplateProxy_CheckExact(T* object) {
